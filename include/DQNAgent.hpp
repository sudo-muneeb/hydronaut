...
public:
    ...
    DQNAgent()
        : m_onlineNet(STATE_DIM, HIDDEN_DIM, ACTION_DIM)
        , m_targetNet(STATE_DIM, HIDDEN_DIM, ACTION_DIM)
        , m_optimizer(m_onlineNet->parameters(),
                      torch::optim::AdamOptions(LR))
        , m_epsilon(EPS_START)
        , m_rng(std::random_device{}())
        , m_dis01(0.f, 1.f)
        , m_disAction(0, ACTION_DIM - 1)
    {
    ...
    int choose_action(const std::vector<float>& state) {
        ...
        auto q_vals = m_onlineNet->forward(t);   // [1, ACTION_DIM]
    ...
    void learn(const std::vector<Experience>& batch) {
        ...
        states_v.reserve(N * STATE_DIM);
        next_states_v.reserve(N * STATE_DIM);
        rewards_v.reserve(N);
        actions_v.reserve(N);
        dones_v.reserve(N);
        ...
        auto states      = torch::from_blob(states_v.data(),
                                {N, STATE_DIM}, torch::kFloat32).clone();
        ...
        torch::Tensor target_q;
        {
            ...
            target_q    = rewards + GAMMA * next_q * (1.f - dones);
        }
...
private:
    ...
    void softUpdateTarget() {
        ...
        auto target_params = m_targetNet->parameters();
        for (std::size_t i = 0; i < online_params.size(); ++i)
            target_params[i].copy_(TAU * online_params[i] +
