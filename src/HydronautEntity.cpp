...
HydronautEntity::HydronautEntity(sf::Vector2u windowSize) {
...
void HydronautEntity::reset(sf::Vector2u windowSize) {
...
bool HydronautEntity::applyCommand(int action, bool dashRequested) noexcept {
...
void HydronautEntity::update(float dt) noexcept {
    m_state->update(*this, dt);
}

// ─── Draw ─────────────────────────────────────────────────────────────────────
void HydronautEntity::draw(sf::RenderWindow& window) {
...
// ─── State transition ─────────────────────────────────────────────────────────
void HydronautEntity::changeState(std::unique_ptr<IEntityState> newState) noexcept {
    m_dashActive = (dynamic_cast<DashState*>(newState.get()) != nullptr);
    m_state = std::move(newState);
}

// ─── Hitboxes ─────────────────────────────────────────────────────────────────
sf::FloatRect HydronautEntity::getBounds() const noexcept {
    return m_sprite.getGlobalBounds();
}

sf::FloatRect HydronautEntity::getGrazeBounds() const noexcept {
...
// ─── Dash state queries ────────────────────────────────────────────────────────
bool HydronautEntity::isDashing() const noexcept { return m_dashActive; }

bool HydronautEntity::dashAvailable() const noexcept { return isDashAvailable(); }
bool HydronautEntity::isDashAvailable() const noexcept {
    if (!m_dashFired) return true;
    return m_dashClock.getElapsedTime().asSeconds() >= DASH_COOLDOWN_SEC;
}

float HydronautEntity::dashCooldownRemaining() const noexcept {
    if (!m_dashFired) return 0.f;
    float el = m_dashClock.getElapsedTime().asSeconds();
    return std::max(0.f, DASH_COOLDOWN_SEC - el);
}

sf::Vector2f HydronautEntity::getPosition() const noexcept {
    auto b = getBounds();
    return { b.left + b.width * 0.5f, b.top + b.height * 0.5f };
}

// ─── State-accessible physics ─────────────────────────────────────────────────
void HydronautEntity::integratePosition() noexcept {
    m_sprite.move(m_vel);
}

void HydronautEntity::clampToWindow() noexcept {
...
void HydronautEntity::tickParticles(float dt) noexcept {
    sf::FloatRect b = getBounds();
...
void HydronautEntity::emitDashParticles(float dt) noexcept {
    sf::FloatRect b = getBounds();
    sf::Vector2f  rear(b.left, b.top + b.height * 0.5f);
    m_particles.emit(rear, m_vel, 1);
...
void HydronautEntity::spawnDashTrail(int count) {
    sf::FloatRect b = getBounds();
    sf::Vector2f  rear(b.left, b.top + b.height * 0.5f);
    m_particles.emit(rear, m_vel, count);
}

void HydronautEntity::markDashFired() noexcept {
    m_dashFired = true;
    m_dashActive = true;
    m_dashClock.restart();
}
...
// ─── Debug ────────────────────────────────────────────────────────────────────
void HydronautEntity::drawDebugHitboxes(sf::RenderWindow& window) const {
...
// ─── Memento Pattern API ────────────────────────────────────────────────────
HydronautEntity::Snapshot HydronautEntity::saveState() const {
    Snapshot snap;
...
void HydronautEntity::restoreState(const Snapshot& snap) {
